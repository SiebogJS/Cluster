...
DefaultClusterManager.prototype.initHost = function (callback) {
...
DefaultClusterManager.prototype.register = function () {
    ...
    var message = HostMessage(this.host.alias, {address: address[0], port: address[1]}, LayerEnum.HOST, msg);
    
    this.host.send({address: address[0], port: address[1]}, message.stringify());
};

DefaultClusterManager.prototype.initEndpoints = function(callback) {
    ...
    NetworkUtil.getRandPort(function (error, port) {
        ...
        if(!error) {
            self.host.listen(port, EndpointTypeEnum.ROUTER, callback);
...
DefaultClusterManager.prototype.publish = function (msg, topic, layer) {
    this.host.publish(HostMessage(self.host.alias, {}, layer || LayerEnum.MESSAGE_MANGER, msg).stringify(), topic);
};

DefaultClusterManager.prototype.send = function (hostAlias, msg, layer) {

    for (var i = 0; i < this.host.registeredHosts.length; i++) {

        if (this.host.registeredHosts[i].alias === hostAlias) {

            var host = this.host.registeredHosts[i];

            for(var j = 0; j < host.endpoints.length; i++) {

                if (host.endpoints[j].type === EndpointTypeEnum.ROUTER) {

                    var endpoint = host.endpoints[j];
                    var receiver = {address: endpoint.address, port: endpoint.port};

                    var message = HostMessage(self.host.alias, receiver, layer || LayerEnum.MESSAGE_MANGER, msg);
                    this.host.send(receiver, message.stringify());
...
DefaultClusterManager.prototype.initHandlers = function () {
    ...
    self.addHandler(HostMessageTypeEnum.REGISTER_HOST_REQUEST, function (sender, newHost) {
        ...
        if (self.host.registerHost(newHost)) {
            ...
            self.host.publish(HostMessage(self.host.alias, {}, LayerEnum.HOST, pubmsg).stringify(), 'PENDING');
            ...
            for(var i = 0; i < newHost.endpoints.length; i++) {
                ...
                if(endpoint.type === EndpointTypeEnum.ROUTER){
                    ...
                    var message = HostMessage(self.host.alias, receiver, LayerEnum.HOST, msg);
